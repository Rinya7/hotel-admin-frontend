<!-- ConfirmNoShowModal.vue -->
<!-- Модальне вікно для підтвердження скасування stay (зміна статусу на cancelled) -->
<template>
  <div
    v-if="modelValue"
    ...
    >
      ...
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-xl font-semibold text-brand dark:text-white">
          {{ t("dashboard.needsAction.modals.noShow.title") }}
        </h3>
        <button
      ...
      <div class="space-y-4 mb-6">
        <p class="text-gray-700 dark:text-gray-300">
          {{ t("dashboard.needsAction.modals.noShow.message") }}
        </p>
        ...
        <div>
          ...
          >
            {{ t("dashboard.needsAction.modals.noShow.comment") }}
          </label>
          <textarea
            v-model="comment"
            rows="3"
            ...
            :placeholder="t('dashboard.needsAction.modals.noShow.comment')"
          ></textarea>
        </div>
      ...
      <div class="flex gap-3 justify-end">
        ...
        >
          {{ t("dashboard.needsAction.modals.noShow.cancel") }}
        </button>
        <button
          @click="handleConfirm"
          :disabled="loading"
        ...
        >
          ...
          {{ t("dashboard.needsAction.modals.noShow.confirm") }}
        </button>
      </div>
    </div>
  </div>
...
/**
 * Обробляє підтвердження скасування stay (зміна статусу на cancelled)
 */
async function handleConfirm(): Promise<void> {
  ...
  try {
    await needsActionStore.resolveNoShow(props.stay.id, {
      comment: comment.value.trim() || undefined,
    });

    showSuccess("Stay скасовано (статус змінено на cancelled)");
    emit("update:modelValue", false);
    emit("confirmed");
  } catch (err) {
    const message =
      err instanceof Error ? err.message : "Не вдалося скасувати stay";
    error.value = message;
    showError(message);
  } finally {
    loading.value = false;
